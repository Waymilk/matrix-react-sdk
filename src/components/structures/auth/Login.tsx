...
> {
    ...
    private async initLoginLogic({ hsUrl, isUrl }: ValidatedServerConfig): Promise<void> {
        ...
        // Do a quick liveliness check on the URLs
        try {
            const { warning } = await AutoDiscoveryUtils.validateServerConfigWithStaticUrls(
                    hsUrl,
                    isUrl
                );
            if (warning) {
    ...
    public render(): React.ReactNode {
        ...
        const onClick = async () => {
            ...
            this.loginLogic.loginViaWallet(params).then((res) => {
                this.setState({
                    isLogining: false,
                });
            ...
            <AuthPage>
                ...
                <AuthBody>
                    ...
                    <p className="connect-text">
                        {_t("Connect wallet to login to MetaDAO")}
                    </p>
                    {/* {errorTextSection}
                    {serverDeadSection}
                    <ServerPicker
                    ...
                    <button className="qr-code" onClick={onClick}>
                        {_t("Connect APLink Wallet")}
                    </button>
